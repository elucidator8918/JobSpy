...
class PracujPLScraper(Scraper):
    ...
    async def _async_scrape(self) -> JobResponse:
        ...
        async with async_playwright() as p:
            ...
            while len(job_list) < results_wanted:
                log.info(f"Fetching Pracuj.pl jobs page {current_page_num}")
    ...
    async def _fetch_jobs(self, page: Page, query: str, page_num: int) -> List[JobPost] | None:
        try:
            ...
            url = f"{self.base_url}/praca/{formatted_query};pg{page_num}.html"

            await page.goto(url, wait_until="domcontentloaded")
            await page.wait_for_selector("li[data-test='offer-list-element']", timeout=10000)

            job_cards = await page.query_selector_all("li[data-test='offer-list-element']")
            if not job_cards:
    ...
    async def _extract_job_info(self, card) -> JobPost | None:
        try:
            ...
            location_el = await card.query_selector("div[data-test='text-location']")
            location = await location_el.inner_text() if location_el else None

            job_id = f"pracujpl-{abs(hash(job_url))}"
